...
7. **尾部作者签名区（固定作者签名，仅末尾一处）**：
   - 展示公众号卡片：`https://cdn.undsky.com/img/card_01_sketch_notes_raw_thumb.png`（居中展示卡片图片）
   - 第二句（互动引导，通用可保留原样）：`如果你觉得今天这篇有收获，欢迎**点赞、分享、推荐、写留言**，我们下篇见`
   - **原文末尾已有作者签名段**（如"我是 XXX…"）→ 直接沿用原文的署名。
...
10. **外链转引用（文末引用链接）**：由于微信公众号正文不支持直接点击外部超链接，正文中出现的外部链接/Markdown 超链接（如 `[文字](https://...)`）**必须转为引用标记**（如 `文字 [2]` 或上标），并在文末生成统一的「引用链接」卡片（固定第一项为 `[1] 访问 Undsky: https://www.undsky.com`，其余外链按出现顺序从 `[2]` 开始顺延编号列出）。
...
- **签名区有且仅有末尾一个**：用固定文案，不在中间或多处出现；原文末尾若已有作者签名/"点赞、分享、推荐、写留言"类段落，识别并**并入**这唯一的签名区/CTA 卡片，不要保留原文段又再生成一个。
